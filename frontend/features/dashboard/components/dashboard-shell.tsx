...
const navigation = [
  ...
  { label: "Services", icon: Command },
  { label: "Incidents", icon: ShieldAlert },
  { label: "Uptime", icon: HeartPulse },
  { label: "Analytics", icon: ChartSpline },
];
          ...
          <nav className="space-y-1 p-4">
            {navigation.map(({ label, icon: Icon, active }) => (
              <div
                key={label}
                className={cn(
                  "flex items-center gap-3 rounded-2xl px-4 py-3 text-sm text-slate-400",
        ...
        <div className="mb-6 flex gap-2 overflow-x-auto pb-1 lg:hidden">
          {navigation.map(({ label, active }) => (
            <div
              key={label}
              className={cn(
                "whitespace-nowrap rounded-full border border-white/10 px-4 py-2 text-sm text-slate-400",
